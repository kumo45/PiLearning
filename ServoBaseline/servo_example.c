...
int main(void)
{
  ...
  buffer[0] = 0x0C;   // "stop time" reg for channel 0
  buffer[1] = 1250 & 0xff; // The "low" byte comes first...
  buffer[2] = (1250>>8) & 0xff; // followed by the high byte.
  write(file_i2c, buffer, length); // Initiate the write.
  ...
  buffer[0] = 0x08;   // "stop time" reg for channel 0
  buffer[1] = 2000 & 0xff; // The "low" byte comes first...
  buffer[2] = (2200>>8) & 0xff; // followed by the high byte.
  write(file_i2c, buffer, length); // Initiate the write.
   buffer[0] = 0x0C;   // "stop time" reg for channel 0
  buffer[1] = 2000 & 0xff; // The "low" byte comes first...
  buffer[2] = (2200>>8) & 0xff; // followed by the high byte.
  ...
  while(1)
  {
    ...
    angle += increase;
    buffer[0] = 0x08;   // "stop time" reg for channel 0
    buffer[1] = angle & 0xff; // The "low" byte comes first...
    buffer[2] = (angle>>8) & 0xff; // followed by the high byte.
    write(file_i2c, buffer, length);
